#!/usr/bin/env node
/**
 * Offline parser for August 20 FinancialJuice snapshot (blind unseen-day input).
...
const rawPath = path.join(root, 'replay-fixtures', 'aug20-financialjuice-snapshot-raw.txt');
const outPath = path.join(root, 'replay-fixtures', 'financialjuice-2026-08-20-ai-replay.json');
const statsPath = path.join(root, 'replay-fixtures', 'aug20-financialjuice-parse-stats.json');
...
const retained: ParsedRow[] = units
    ...
    .map((unit, index) => {
        const row: ParsedRow = {
            time: unit.time,
            source: 'FinancialJuice',
            guid: `aug20fj${String(index + 1).padStart(5, '0')}`,
...
console.log('Sample first 3:', retained.slice(0, 3).map((r) => `${r.time} | ${r.headline.slice(0, 80)}`));
console.log('Sample last 3:', retained.slice(-3).map((r) => `${r.time} | ${r.headline.slice(0, 80)}`));
