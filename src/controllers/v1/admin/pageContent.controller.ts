...
export const getPageContent = async (req, res, next) => {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse('Page content retrieved successfully', content));
    }
    catch (error) {
        next(error);
    }
...
export const updatePageContent = async (req, res, next) => {
    try {
        ...
        if (sectionId) {
            ...
            content = await pageContentRepository.findByPageIdentifierAndSectionKey(pageIdentifier, key);
        }
        else {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(SUCCESS_MESSAGES.UPDATED));
    }
    catch (error) {
        next(error);
    }
