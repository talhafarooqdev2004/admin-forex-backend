...
export const getAllPaymentGateways = async (req, res, next) => {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse('Payment gateways retrieved successfully', gateways));
    }
    catch (error) {
        next(error);
    }
...
export const updatePaymentGateway = async (req, res, next) => {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse(SUCCESS_MESSAGES.UPDATED, gateway));
    }
    catch (error) {
        next(error);
    }
...
export const toggleActivePaymentGateway = async (req, res, next) => {
    try {
        ...
        res.status(HTTP_STATUS.OK).json(successResponse('Payment gateway status updated successfully', gateway));
    }
    catch (error) {
        next(error);
    }
